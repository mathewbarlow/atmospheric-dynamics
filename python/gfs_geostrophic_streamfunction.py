...
# run on python 3.7
#
# python code to compare 500hPa geostrophic and rotational winds
...
# calculate streamfunction and rotational wind froom global fields
uglobe = ds['u-component_of_wind_isobaric'][0, iz, :, :].values
vglobe = ds['v-component_of_wind_isobaric'][0, iz, :, :].values
w = VectorWind(uglobe, vglobe)
psi, chi = w.sfvp()
psiglobe = psi-np.mean(psi)
upsiglobe, vpsiglobe = w.nondivergentcomponent()

# get variablles for specified lat-lon region and level
...
upsi = upsiglobe[iy2:iy1, ix1:ix2]
vpsi = vpsiglobe[iy2:iy1, ix1:ix2]
psi = psiglobe[iy2:iy1, ix1:ix2]
...
upsi = np.flip(upsi, axis=0)
vpsi = np.flip(vpsi, axis=0)
...
plt.title(format(level,'g')+'hPa SMOOTH HEIGHT, GEOSTROPHIC WINDS\n'+myhour+'Z '+fdate)
...
gl.xlocator = mticker.FixedLocator(np.arange(lon1-360, lon2-360+10, 10))
gl.ylocator = mticker.FixedLocator(np.arange(lat1, lat2+5, 5))
plt.title(format(level,'g')+'hPa HEIGHT, WINDS\n'+myhour+'Z '+fdate)
...
clevs = np.arange(-7.1e7*0.98, 1.7e7*1.016, 7.9e6)

plt.contour(lons, lats, psi, clevs, 
            transform=ccrs.PlateCarree(),
            colors='black', linewidths=0.5, linestyles='solid')

sk = 15
plt.quiver(lons[::sk], lats[::sk], upsi[::sk, ::sk], vpsi[::sk, ::sk],
           transform=ccrs.PlateCarree(), scale=1e3, color='black')
...
gl.xlocator = mticker.FixedLocator(np.arange(lon1-360, lon2-360+10, 10))
gl.ylocator = mticker.FixedLocator(np.arange(lat1, lat2+5, 5))
plt.title(format(level,'g')+'hPa STREAMFUNCTION, ROTATIONAL WINDS\n'+myhour+'Z '+fdate)
